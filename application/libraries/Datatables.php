      ...
      if($sSearch != '')
        for($i = 0; $i < count($mColArray); $i++)
          if($this->ci->input->post('bSearchable_' . $i) == 'true' && in_array($mColArray[$i], $columns)) {
            $params = explode(".",$this->select[$mColArray[$i]]);
            $result_count = count($params);
            if ($result_count>1 and $result_count <=2){
                $params[0]=  str_replace('DISTINCT(', '', $params[0]);
                $params[1]=  str_replace(')', '', $params[1]); 
                $sWhere .= "UPPER(\"".$params[0]."\".\"" .$params[1]. "\")  LIKE UPPER('%" . $sSearch . "%') OR ";
            } else {
                $sWhere .= "UPPER(".$this->select[$mColArray[$i]].")" . " LIKE UPPER('%" . $sSearch . "%') OR ";
            }
            
            //echo $this->select[$mColArray[$i]].' ';
          }
