...
class Ion_auth
{
	...
	public function forgotten_password($identity)    //changed $email to $identity
	{
		if ( $this->ion_auth_model->forgotten_password($identity) )   //changed
		{
            ...
            $user = $this->where($this->config->item('identity', 'ion_auth'), $identity)->where('ACTIVO', 1)->users()->row();  //changed to get_user_by_identity from email
			if ($user)
			{
				$data = array(
	...
	public function forgotten_password_complete($code)
	{
		...
		$profile  = $this->where('REMEMBER_CODE', $code)->users()->row(); //pass the code to profile
	...
	{   
		$code="'".$code."'";
		$this->data['result'] = $this->ion_auth_model->getuser("PASSWORDCODE =".$code);
		//$consult= $this->where('PASSWORDCODE', $code);
		


		$profile =$this->data['result'];  //$consult->users()->row(); //pass the code to profile
	 ...
	 * @author Mathew
	 **/
	public function register($cedula,$username, $password, $email, $additional_data = array(), $group_ids = array()) //need to test email activation
	{
		...
		if (!$email_activation)
		{
			$id = $this->ion_auth_model->register($cedula,$username, $password, $email, $additional_data, $group_ids);
	...
	/*
	*Funcion que crea el usuario sin el uso del mail
	*/
		/**
	 * logout
	 *
	...
	//	print_r("entrando");
		...
		//$session_id = (bool)$this->session->userdata('email');
		// $user=(bool)$this->session->userdata('identity');
