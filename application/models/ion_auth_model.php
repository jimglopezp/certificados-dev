...
class Ion_auth_model extends CI_Model {
   ...
   * @author Mathew
   * */
  public function change_password2($identity, $old, $new)
  {
    $this -> trigger_events('pre_change_password');
  ...
  /*
  *Registro de usuario sin usar el email
  *
  */
  public function registers($cedula, $username, $password, $additional_data = array(), $groups)
  {
    $this -> trigger_events('pre_register');
    $manual_activation = $this -> config -> item('manual_activation', 'ion_auth');
   ...
   * @author Mathew
   * */
  public function login($identity, $password, $remember , $datos )
  {
    ...
    if ($datos == 0) {
      ...
      $user = $datos;
      $password;
      $password = $this->hash_password_db($datos, $password);
   //   echo "<pre>";print_r(  $user );exit("</pre>");

      /*   echo $query->result_array[0]['CODEMPRESA'];
      echo "---";
      echo  sha1( $contrasena);die;*/
      //echo  $password;die;
      $this->trigger_events('extra_where');
      ...
      if ($password === FALSE) 
      {
        ...
        return array("Mensaje" => "Contrase&ntilde;a incorrecta", "Respuesta" => 4);
      }

      if ($password === TRUE) {
      
        
          $regional = $datos['COD_REGIONAL'];
        //  echo "<pre>";print_r( $regional);exit("</pre>");
        

        $session_data = array(
          //  'identity' => $user -> IDENTITY_COLUMN,
            'username' => $user ['REPRESENTANTE_LEGAL'],
            'email' => $user ['CORREOELECTRONICO'],
            'user_id' => $user ['CODEMPRESA'],//everyone likes to overwrite id so we'll use user_id
      ...
      // echo "<pre>";print_r($session_data);exit("</pre>");
       ...
       // $this -> update_last_login($user['CODEMPRESA']);
  ...
  public function user($id = NULL)
  {
    ...
    $this -> limit(2);
    $this -> where('EMPRESA' . '.CODEMPRESA', $id);
